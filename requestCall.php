...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $emailTemplate = '
                    ...
                    <table align="center" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; margin:auto; background:#ffffff; border-radius:12px; box-shadow:0 4px 10px rgba(0,0,0,0.1);">
                        ...
                        <td style="background:#1d4ed8; padding:20px; border-radius:12px 12px 0 0; text-align:center; color:#ffffff;">
                            <h1 style="margin:0; font-size:24px;">Call Request</h1>
                        </td>
                        </tr>
                        <tr>
    ...
    if ($conn->connect_error) {
        ...
        if (!empty($full_name) and !empty($phone)) {
            $stmt = $conn->prepare("insert into call_request(full_name,phone,message) values(?,?,?)");
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    // Recipients
                    $mail->setFrom('devansh@dentigolab.com', 'Skydent Designs');
                    ...
                    // Content
                    $mail->isHTML(true);
                    $mail->Subject = 'Call request from ' . $full_name;
